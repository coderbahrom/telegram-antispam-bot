...
async def on_join(event: ChatMemberUpdated, bot: Bot) -> None:
    ...
    # Profil tekshiruvi (guruh ham, KANAL ham): matn + rasm + kombinatsiya —
    # spam-profil bo'lsa darrov ban (xabari toza bo'lsa ham — masalan "Salom")
    state.mark_profile_checked(user.id)
    if await ban_if_profile_spam(bot, chat.id, user):
        return
